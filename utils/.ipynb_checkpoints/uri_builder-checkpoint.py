...
class UriBuilder():
    ...
    def _clean_arxiv_id(self, url_abs):
        if not url_abs:
            return None
        match = re.search(r'arxiv\.org/abs/(\d{4,5}\.\d{4,5})(v\d+)?', url_abs)
        if match:
    ...
    def build_modelcard_uri(self, paper_id):
        ...
        clean_paper_id = self._clean_arxiv_id(paper_id)

        return f"{self.BASE_URI}/model/{clean_paper_id}"
